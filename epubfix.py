...
def heal_epub(epub_path: Path, config: dict) -> (Path, dict):
    ...
    try:
        ...
        if opf_tree is not None:
            # Capture the metrics dictionary
            metrics = epubstandard.process_epub(unzip_dir, opf_tree, opf_path, config)
            repack_epub(unzip_dir, healed_path)
        else:
            logging.error(f"Skipping healing for {epub_path.name} as OPF could not be found.")
            return epub_path, metrics # Return original path if it cannot be processed

    finally:
        shutil.rmtree(unzip_dir, ignore_errors=True)

    return healed_path, metrics
...
def process_with_fix(epub_path: Path, config: dict) -> dict:
    ...
    healed_path, metrics = heal_epub(epub_path, config)
    if healed_path == epub_path: # Healing was skipped
         return {"status": "OK", "before_errors": be, "before_warnings": bw, "after_errors": be, "after_warnings": bw, "used_healed": False, **metrics}
    ...
    # Combine epubcheck results with processing metrics
    final_result = {
        "status": "OK",
    ...
    final_result.update(metrics) # Add all metrics from epubstandard
    return final_result