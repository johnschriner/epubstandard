...
def set_noteref_semantics(anchor_el):
    """
    Mark an <a> as a noteref in a namespace-safe way.
    - Prefer namespaced attribute {OPS_NS}type="noteref" (Clark notation).
    - If serializer or tree rejects that, fall back to adding class "noteref".
    """
    # Always ensure class contains 'noteref'
    cls = anchor_el.get('class', '').strip()
    parts = set(cls.split()) if cls else set()
    if 'noteref' not in parts:
        parts.add('noteref')
        anchor_el.set('class', ' '.join(sorted(parts)))
    # Try to set the namespaced attribute
    try:
        anchor_el.set(f'{{{OPS_NS}}}type', 'noteref')
    except Exception:
        # Some HTML trees/serializers may reject namespaced attrs; class is enough.
        pass
...
def find_noterefs(root: html.HtmlElement) -> List[html.HtmlElement]:
    """Return candidate in-text note refs without requiring a bound 'epub' prefix.
    We detect:
      - <sup>…</sup>
      - any element with an attribute whose local-name() == 'type' and namespace-uri()== OPS and value == 'noteref'
      - elements whose class contains 'noteref'
    """
    refs: List[html.HtmlElement] = []
    # 1) classic <sup>
    refs.extend(root.xpath('//sup'))
    # 2) epub:type="noteref" (namespace-agnostic)
